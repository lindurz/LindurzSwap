...
contract Vitrum {
    ...
    modifier nonReentrant() {
        require(!_reentrancyLock, "VTX: REENTRANCY_DETECTED");
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "VTX: NOT_OWNER");
        _;
    }
    ...
    function sendNative(address payable to, uint256 amount) public onlyOwner nonReentrant {
        require(address(this).balance >= amount, "VTX: INSUFFICIENT_NATIVE");
        (bool success, ) = to.call{value: amount}("");
        require(success, "VTX: NATIVE_TRANSFER_FAILED");
    ...
    function transferERC20(address tokenAddress, address to, uint256 amount) public onlyOwner nonReentrant {
        ...
        require(success, "VTX: ERC20_TRANSFER_FAILED");
    }
    ...
    function transferAllERC20(address tokenAddress, address to) public onlyOwner nonReentrant {
        ...
        require(balance > 0, "VTX: NO_ERC20_BALANCE");
    ...
    function _transfer(address from, address to, uint256 value) internal {
        require(from != address(0), "VTX: FROM_ZERO_ADDRESS");
        require(to != address(0), "VTX: TO_ZERO_ADDRESS");

        uint256 fromBalance = _balances[from];
        require(fromBalance >= value, "VTX: INSUFFICIENT_BALANCE");
    ...
    function _spendAllowance(address owner_, address spender, uint256 value) internal {
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= value, "VTX: INSUFFICIENT_ALLOWANCE");
            unchecked {
    ...
    function _approve(address owner_, address spender, uint256 value) internal {
        require(owner_ != address(0), "VTX: APPROVE_FROM_ZERO");
        require(spender != address(0), "VTX: APPROVE_TO_ZERO");
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), "VTX: NEW_OWNER_ZERO");
        owner = newOwner;
    }
