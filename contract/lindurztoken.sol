...
contract LindurzToken {
    // Basic token information
    string public constant name = "Lindurz Token";
    string public constant symbol = "LDRZ";
    uint8 public constant decimals = 18;
    uint256 public constant totalSupply = 120_000_000 * 10**18; // 120 juta LDRZ
    ...
    modifier nonReentrant() {
        require(!_reentrancyLock, "LDRZ: REENTRANCY_DETECTED");
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "LDRZ: NOT_OWNER");
        _;
    }
    ...
    // Receive native token (ETH/BNB/MATIC) - dengan reentrancy guard
    ...
    function sendNative(address payable to, uint256 amount) public onlyOwner nonReentrant {
        require(address(this).balance >= amount, "LDRZ: INSUFFICIENT_NATIVE");
        (bool success, ) = to.call{value: amount}("");
        require(success, "LDRZ: NATIVE_TRANSFER_FAILED");
    ...
    function transferERC20(address tokenAddress, address to, uint256 amount) public onlyOwner nonReentrant {
        ...
        require(success, "LDRZ: ERC20_TRANSFER_FAILED");
    }
    ...
    function transferAllERC20(address tokenAddress, address to) public onlyOwner nonReentrant {
        ...
        require(balance > 0, "LDRZ: NO_ERC20_BALANCE");
    ...
    function _transfer(address from, address to, uint256 value) internal {
        require(from != address(0), "LDRZ: FROM_ZERO_ADDRESS");
        require(to != address(0), "LDRZ: TO_ZERO_ADDRESS");

        uint256 fromBalance = _balances[from];
        require(fromBalance >= value, "LDRZ: INSUFFICIENT_BALANCE");
    ...
    function _spendAllowance(address owner_, address spender, uint256 value) internal {
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= value, "LDRZ: INSUFFICIENT_ALLOWANCE");
            unchecked {
    ...
    function _approve(address owner_, address spender, uint256 value) internal {
        require(owner_ != address(0), "LDRZ: APPROVE_FROM_ZERO");
        require(spender != address(0), "LDRZ: APPROVE_TO_ZERO");
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), "LDRZ: NEW_OWNER_ZERO");
        owner = newOwner;
    }
